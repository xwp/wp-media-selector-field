...
class XTeam_Media_Selector_Field {
	...
	public static function enqueue_assets() {
		...
		wp_enqueue_media();
		wp_enqueue_script( 'xteam-media-selector', plugin_dir_url( __FILE__ ) . '/media-selector-field.js', array( 'jquery-ui-sortable' ), self::VERSION, true );
		wp_localize_script( 'xteam-media-selector', 'xteamMediaSelector', self::$entries );
		wp_enqueue_style( 'xteam-media-selector', plugin_dir_url( __FILE__ ) . '/media-selector-field.css', false, self::VERSION );
	}
	...
	public static function the_field( $entry_id, $input_name = '', $current_value = '', $field_id = '' ) {
		...
		?>
			...
			<ul<?php echo $list_attr // xss ok ?>>
				<?php
					foreach ( $current_value as $attachment_id ) :
						...
						if ( !empty($attachment_id) ) {
							...
							if (
								$entry['type'] === 'image'
								&& $entry['preview_size'] !== 'thumbnail'
								&& $image_src = wp_get_attachment_image_src( $attachment_id, $entry['preview_size'], false )
							) {
								if ( is_numeric($entry['preview_size']) ) {
									$thumb_style = sprintf( ' style="width:%dpx;height:%dpx"', $entry['preview_size'], $entry['preview_size'] );
								}
								else {
									$thumb_style = sprintf( ' style="width:%dpx;height:%dpx"', $image_src[1], $image_src[2] );
								}
							}
						}
						else {
			...
			<p><a href="#" class="button-primary xteam-media-select" data-entryid="<?php echo esc_attr( $entry_id ) ?>" data-fieldid="<?php echo esc_attr( $field_id ) ?>"><?php echo esc_html( $entry['select_button'] ) ?></a></p>
		</div>
		<?php
	}
}
